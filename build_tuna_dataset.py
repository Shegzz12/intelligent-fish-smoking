"""Build remaining-time dataset from tuna fish smoking experiments.

Run:
    pip install -r requirements.txt
    python build_tuna_dataset.py

Writes tuna_smoking_remaining_time_dataset_full.csv (all columns) and
tuna_smoking_remaining_time_dataset.csv (training-ready: no leakage).
"""
...
# Tuna fish data file paths - using the main data file
TUNA_READING_2 = ROOT / "tuna_fish_reading_2"

OUT_FULL = ROOT / "tuna_smoking_remaining_time_dataset_full.csv"
OUT_CSV = ROOT / "tuna_smoking_remaining_time_dataset.csv"
...
# Tuna fish experiment parameters - based on actual data
TUNA_START = pd.Timestamp("2026-08-26 11:07:36")  # Start of main tuna smoking (from tuna_fish_reading_2)
TUNA_END = pd.Timestamp("2026-08-26 11:37:16")    # End of main tuna smoking (approximate, will use actual data end)
TUNA_W0 = 2379.7  # Starting weight in grams (from real data: ~2379.7g)
...
def build_tuna_main() -> pd.DataFrame:
    """Build the main tuna fish smoking dataset from experimental data."""
    # Load the main tuna smoking data from tuna_fish_reading_2
    main_data = load_log(TUNA_READING_2)
    
    # Filter to the smoking period
    df = main_data[
        (main_data.timestamp >= TUNA_START) & 
        (main_data.timestamp <= TUNA_END)
    ].copy()
    
    df["session"] = "tuna_main"
    ...
    df["run_id"] = "tuna_2379g"  # Use actual starting weight in run_id
    df["is_synthetic"] = 0
    
    print(
        f"tuna_2379g  {t_end:.1f} min  {w0:.0f}→{df.weight_corrected_g.iloc[-1]:.0f} g  "
...
def build() -> pd.DataFrame:
    # Build the main tuna fish dataset (real experimental data at ~2379g)
    tuna_main = build_tuna_main()
    
    # Extract parameters for power law scaling from the real data
    m_main = float(tuna_main.start_weight_g.iloc[0])  # ~2379g
    t_main = float(tuna_main.elapsed_smoking_min.iloc[-1])
    q_main = float(tuna_main.target_oven_deg_h.iloc[-1])
    lf_main = float(tuna_main.target_loss_frac.iloc[-1])
    ...
    for mass in SYNTH_LOADS_G:
        ...
        syn = synthesize_from_template(
            tuna_main, mass, t_new, lf_new, run_id=f"tuna_synth_{int(mass)}g"
        )
    ...
    all_runs = pd.concat([tuna_main, *synths], ignore_index=True)
